...
function Channel(namespace) {
	...
	namespace.getSocketByUserId = function (userId) {
		_.forEach(namespace.connected, socket => {
			if (socket.client.user.publicId === userId) {
				return socket;
			}
		});
	};
	...
	namespace.addUserToChannel = function (channel, socket, done) {
		channel.pipeline.create('WebRtcEndpoint', (err, outgoingMedia) => {
			...
			// Notify other channel members of the new user
			namespace.in(channel.doc.publicId).emit('message', {
				id: 'user_joined',
				user: socket.id
			});
	...
	namespace.removeUserFromChannel = function (channel, socket, done) {
		...
		// Notify other channel members of the left user
		namespace.in(channel.doc.publicId).emit('message', {
			id: 'user_left',
			user: socket.id
		});
	...
	namespace.createChannel(namespace.doc.rooms[0]);
}
