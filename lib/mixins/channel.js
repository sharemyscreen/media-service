...
function Channel(namespace) {
	...
	namespace.sendToRoom = function (roomPublicId, data) {
		const channel = namespace.channels[roomPublicId];

		if (!channel) {
			logger.info(`Channel does not exist: ${roomPublicId}`);
			return;
		}

		logger.info(`${roomPublicId}: ${channel.members}`);
		_.each(channel.members, socket => {
			socket.emit('room_scope', data);
		});
	};
		...
		namespace.getKurentoClient((err, kurentoClient) => {
			...
			kurentoClient.create('MediaPipeline', (err, pipeline) => {
				...
				_.each(room.members, member => {
					...
					if (socket) {
						namespace.addUserToChannel(room.publicId, socket, err => {
							if (err) {
								return done(err);
							}
						});
	...
	namespace.addUserToChannel = function (roomPublicId, socket, done) {
		logger.info(`addUserToChannel: roomPublicId: ${roomPublicId}, channels: ${namespace.channels}`);
